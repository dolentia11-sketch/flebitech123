...
Base de datos SQLite local para registrar interacciones, temas y brechas de conocimiento.
"""
...
def detect_topic(query: str) -> str:
    ...
    elif any(k in q for k in ['escala', 'ins', 'vhp', 'grado 0', 'grado 1', 'grado 2', 'grado 3', 'grado 4', 'cordon', 'cordón', 'eritema', 'edema', 'secrecion', 'purulenta']):
        return 'Escalas de Flebitis (INS/VHP)'
    elif any(k in q for k in ['cateter', 'catéter', 'calibre', 'gauge', '24g', '22g', '20g', '18g', '16g', '14g', 'color', 'flujo']):
        return 'Selección de Catéter'
    elif any(k in q for k in ['vancomicina', 'potasio', 'kcl', 'amiodarona', 'ciprofloxacina', 'fenitoina', 'fenitoína', 'ceftriaxona', 'ampicilina', 'calcio', 'dad', 'dextrosa', 'npt', 'furosemida', 'omeprazol', 'claritromicina', 'metronidazol']):
        return 'Medicamentos Específicos'
    elif any(k in q for k in ['ph', 'osmolaridad', 'tonicidad', 'hipotonica', 'isotonica', 'hipertonica', 'hemodilucion', 'hemodilución', '600', '900']):
...
def log_question(query: str, response: str, session_id: str = 'default', had_answer: bool = True, source_docs: str = '', latency_ms: float = 0.0) -> int:
    init_db()
